...
export default function ClassPost({ post, classData }) {
  const navigate = useNavigate();
  const isAssignment = post.type === 'assignment';
  console.log('post', post);
  const handleHeaderClick = () => {
    if (!isAssignment && classData) {
      console.log(classData);
      navigate(`/class/${post.classId}`);
    }
    navigate(`/assignments/${post.assignment._id}`);
  };
  return (
    <div
      className="px-4 py-3 border-t last:border-b cursor-pointer"
    ...
    >
      <div className="flex gap-3 items-start">
        <div className="w-12 h-12 flex items-center justify-center rounded-lg bg-blue-50 text-blue-600 font-semibold text-sm shrink-0">
          {classData.class_code?.slice(0, 2).toUpperCase()}
        </div>
