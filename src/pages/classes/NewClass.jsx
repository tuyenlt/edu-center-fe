...
const NewClass = () => {
    ...
    const [sessionArray, setSessionArray] = useState(course?.sessions_details || []);
    ...
    const isValidSessionArray = (sessions) => {
        return sessions.every((session) => session.start_time && session.end_time && session.room);
    };
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mx-auto mt-6 w-full max-w-screen-xl">
            <div>
            ...
            <div>
                ...
                />
                {errors.class_code && (
                    <p className="text-red-500 text-sm mt-1">{errors.class_code.message}</p>
                )}
            </div>
            ...
            <div className="flex flex-col w-full gap-2">
                ...
                {sessionArray.map((session, index) => {
                    return <SessionInfo session={session} index={index + 1} onClick={() => {
            ...
            <SessionSchedule session={currentSession} index={sessionIndex} onConfirm={handleSetSessionTime} setIsOpen={setScheduleOpen} isOpen={scheduleOpen} />
        </form>
    );
};
