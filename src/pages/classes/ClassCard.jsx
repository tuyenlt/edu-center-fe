...
export function ClassCard({ data }) {
	...
	return (
		<Link to={`/class/${data._id}`}>
			<Card
				className="w-75 h-75 p-0 rounded-xl text-white relative overflow-hidden border-gray-300 hover:hover:shadow-[0_1px_2px_0_rgba(63,63,70,0.3),_0_2px_6px_2px_rgba(63,63,70,0.15)]
 gap-0"
			>
				...
				<CardFooter className="text-black flex flex-col gap-2 justify-between h-full pb-8">
					...
					<div className="self-end flex gap-2">
						<Clock />
						<div className="">{dateTimeConvert_2(upcomingSession.start_time)}</div>
					</div>
				</CardFooter>
			</Card>
		</Link>
