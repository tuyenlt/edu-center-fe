	...
	const fetchBills = async () => {
		try {
			...
			if (user.role === 'manager') {
				...
				// filter bills for manager
				setBills(sortedBills.filter((b) => b.type === "tuition"));
			}
			setPage(1);
		} catch (err) {
	...
	const handleBulkPay = async () => {
		try {
			await api.patch('/bills/pay', {
				...
				payment_details: {
					transactionId:
						paymentMethod === 'banking' ? transactionId : undefined,
			...
			<div className="flex flex-col md:flex-row gap-4 items-center">
				...
				<Dialog open={bulkOpen} onOpenChange={setBulkOpen}>
					<DialogTrigger asChild>
						<Button disabled={selected.size === 0} className="col-span-1">
				...
				<CardContent className="space-y-2">
					...
					{currentBills.length > 0 ? (
						currentBills.map((bill) => (
							<div
								key={bill._id}
								className={`grid grid-cols-12 gap-2 items-center py-2 border-b hover:bg-gray-50 ${bill.status === 'paid' ||
									(selectingType !== bill.type && selectingType !== 'all')
									? 'opacity-50'
									: ''
									}`}
							>
								...
								<div
									className={`col-span-2 ${bill.type === 'tuition' ? 'text-green-500' : 'text-red-500'
										}`}
								>
