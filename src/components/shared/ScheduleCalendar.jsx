...
export default function ScheduleCalendar({
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push(
        <div
          key={i}
          className={`text-center font-medium py-1 cursor-pointer ${
            isToday
                  ...
                  {cellEvents.length > 0 && (
                    <>
                      ...
                      {(() => {
                        ...
                        return (
                          <div
                            className={`absolute z-10 bg-blue-100 border border-blue-400 text-xs p-1 rounded shadow-sm ${
                              ev.prev ? 'bg-gray-300 opacity-50' : ''
                            }`}
  ...
  return (
    <div className="max-w-8xl mx-auto border w-full min-w-5xl">
      <CardContent className="p-0">
        ...
        <div className="max-h-[600px] overflow-y-auto">{renderWeekCells()}</div>
                  ...
                  .map((ev, i) => {
                    ...
                    return (
                      <li key={i} className="text-sm">
                        {format(s, 'HH:mm')} - {format(e, 'HH:mm')} : Room{' '}
                        {ev.room} {ev.class_name ? `- ${ev.class_name}` : ''} -{' '}
                        {ev.title}
                      </li>
                    );
                  })}
