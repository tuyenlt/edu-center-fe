      ...
      days.push(
        <div
          ...
          className={`text-center font-medium px-2 py-1 ${
            isToday ? 'bg-blue-200 text-blue-800 font-bold' : 'text-gray-600'
          }`}
        >
          {format(day, 'EEE d')}
        </div>
          ...
          <div key={hourIndex} className="grid grid-cols-8 border border-t-0">
            ...
            {Array.from({ length: 7 }).map((_, dayIndex) => {
              ...
              return (
                <div
                  key={cellKey}
                  className={`p-0 relative cursor-pointer border-r last:border-r-0 transition-all h-12 ${
                    isSelected ? 'bg-blue-300' : 'hover:bg-blue-50'
                  }`}
                  onClick={() => {
                  ...
                  {scheduleData.map((item, i) => {
                    ...
                    if (
                      ...
                      return (
                        ...
                        >
                          <div className="font-medium text-blue-900 truncate text-wrap">
                            Room {item.room} - {item.class_name || ''}
                          </div>
                          <div className="text-[10px] text-blue-800 truncate">
                            {item.title}
                          </div>
                          <div className="text-[10px] text-muted-foreground">
                            {format(itemStart, 'HH:mm')} -{' '}
                            {format(itemEnd, 'HH:mm')}
                          </div>
                        </div>
                      );
                    }
