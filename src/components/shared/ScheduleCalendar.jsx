	...
	const renderDays = () => {
		...
		for (let i = 0; i < 7; i++) {
			...
			days.push(
				<div
					key={i}
					className={`text-center font-medium px-2 py-1 cursor-pointer ${isToday
					...
					<div key={hIdx} className="grid grid-cols-8 border border-t-0">
						...
						{Array.from({ length: 7 }).map((_, dIdx) => {
							...
							const isInsideExistingGroup = groups.some(
								(g) =>
									g.start.getHours() < hour && g.end.getHours() > hour
							);
												...
												>
													...
													{g.events.map((ev, idx) => {
														...
														return (
															<div
																key={idx}
																className={`mb-0.5 ${ev.prev ? 'opacity-50' : 'opacity-100'}`}
																title={`Room ${ev.room} • ${ev.title} • ${format(
																	s,
																	'HH:mm'
																)}–${format(e, 'HH:mm')}`}
															>
